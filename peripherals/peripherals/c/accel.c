...
static __INLINE uint8_t accel_reg_read(uint8_t reg)
{
  // ADD CODE
	...
	transmit[1] = 0x00;
	accel_CSN_low();	
	spiTx(ACCEL_SPI_BASE, transmit, 2, receive);
	accel_CSN_high();
  return receive[1]; // Modify to return the register value
}
...
int16_t accel_read_x(void)
{
  // ADD CODE
	...
	uint8_t higher_x;
	uint16_t total_x;
	lower_x = accel_reg_read(ACCEL_OUTX_L_XL);
	higher_x = accel_reg_read(ACCEL_OUTX_H_XL);
	total_x = ((higher_x  << 8) | lower_x);
  return total_x; // Modify to return the x value
...
int16_t accel_read_y(void)
{
  // ADD CODE
	...
	uint8_t higher_y;
	uint16_t total_y;
	lower_y = accel_reg_read(ACCEL_OUTY_L_XL);
	higher_y = accel_reg_read(ACCEL_OUTY_H_XL);
	total_y = ((higher_y  << 8) | lower_y);
  return total_y; // Modify to return the Y value
...
int16_t accel_read_z(void)
{
  // ADD CODE
	...
	uint8_t higher_z;
	uint16_t total_z;
	lower_z = accel_reg_read(ACCEL_OUTZ_L_XL);
	higher_z = accel_reg_read(ACCEL_OUTZ_H_XL);
	total_z = ((higher_z  << 8) | lower_z);
  return total_z; // Modify to return the Z value
